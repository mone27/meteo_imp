...
# %% auto 0
__all__ = ['PosDef', 'is_symmetric', 'symmetric_upto', 'is_posdef', 'is_posdef2', 'check_posdef', 'KalmanFilter']
...
# %% ../../lib_nbs/kalman/00_Kalman_Filter.ipynb 20
def is_symmetric(value, atol=1e-5):
    return torch.isclose(value, value.mT, atol=atol).all(-2).all(-1)
...
# %% ../../lib_nbs/kalman/00_Kalman_Filter.ipynb 32
def is_posdef2(cov):
...
def check_posdef(value, error=True):
    is_pd = torch.linalg.cholesky_ex(value).info.eq(0)
    if not is_pd:
        msg = f"not pos definite \n {value}"
        if error:
...
# %% ../../lib_nbs/kalman/00_Kalman_Filter.ipynb 60
...
# %% ../../lib_nbs/kalman/00_Kalman_Filter.ipynb 65
...
# %% ../../lib_nbs/kalman/00_Kalman_Filter.ipynb 70
...
# %% ../../lib_nbs/kalman/00_Kalman_Filter.ipynb 79
@torch.no_grad()
@patch
...
# %% ../../lib_nbs/kalman/00_Kalman_Filter.ipynb 102
@patch
...
# %% ../../lib_nbs/kalman/00_Kalman_Filter.ipynb 109
@patch
...
# %% ../../lib_nbs/kalman/00_Kalman_Filter.ipynb 113
@patch
def filter_loglikelihood(self: KalmanFilter, obs):
    ...
    for t in range(max_t):
        if obs_mask[t]:
            ...
            check_posdef(pred_obs_cov) # this has higher tollerance than pytorch default
            ll = MultivariateNormal(pred_obs_mean, pred_obs_cov, validate_args=False).log_prob(obs[t])
            lls[t] = ll

    return lls.sum()

# %% ../../lib_nbs/kalman/00_Kalman_Filter.ipynb 119
@patch
def loglikelihood(self: KalmanFilter,
    ...
    for t in range(len(times)):
        check_posdef(covs[t]) # this has higher tolerance than 
        lls[t] = MultivariateNormal(means[t], covs[t], validate_args=False).log_prob(obs_test[t:t+1])
    return lls.sum() 
        

# %% ../../lib_nbs/kalman/00_Kalman_Filter.ipynb 124
@patch
